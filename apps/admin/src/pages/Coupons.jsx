...
function CouponPreview({ form }) {
  ...
  const savingsText = (() => {
    if (form.discount_type === 'flat') {
      ...
      if (v > 1000) return `Save Rs.${v} (large discount)`;
      return `Save Rs.${v}`;
    }
    if (form.discount_type === 'percent') {
      ...
      return `Save ${v}%${cap ? ` (up to Rs.${cap})` : ''}`;
    }
...
export default function Coupons() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false); setEditingId(null); setForm(EMPTY_FORM); fetchCoupons();
  ...
  const formatDiscountValue = (c) => {
    if (c.discount_type === 'flat') return `Rs.${Number(c.discount_value)} off`;
    if (c.discount_type === 'percent') return `${Number(c.discount_value)}% off${c.max_discount_amount ? ` (max Rs.${Number(c.max_discount_amount)})` : ''}`;
  ...
  return (
    <div className="coupons-page">
      <div className="coupons-header">
        ...
        <button className="btn-primary" onClick={() => { setForm(EMPTY_FORM); setEditingId(null); setShowForm(true); setFormError(null); }}>+ Create Coupon</button>
      </div>
              ...
              {coupons.map(c => (
                <tr key={c.id}>
                  ...
                  <td><div className="coupon-actions">
                    ...
                    <button className="btn-icon btn-danger" onClick={() => handleDelete(c.id)}>Delete</button>
                  </div></td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="coupon-form-modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="coupon-form-body">
              ...
              <fieldset><legend>Discount</legend>
                ...
                {form.discount_type !== 'free_delivery' && <div className="form-row"><label>Value {form.discount_type === 'flat' ? '(Rs.)' : '(%)'} *</label><input type="number" min="0" step="0.01" value={form.discount_value} onChange={e => handleFormChange('discount_value', e.target.value)} required /></div>}
                {form.discount_type === 'percent' && <div className="form-row"><label>Max Discount (Rs.)</label><input type="number" min="0" step="0.01" value={form.max_discount_amount} onChange={e => handleFormChange('max_discount_amount', e.target.value)} placeholder="Cap" /></div>}
              </fieldset>
              ...
              <fieldset><legend>Schedule</legend>
                <div className="form-row-2">
                  <div className="form-row"><label>Starts At</label><input type="datetime-local" value={form.starts_at} onChange={e => handleFormChange('starts_at', e.target.value)} /></div>
                  <div className="form-row"><label>Ends At (Valid Till)</label><input type="datetime-local" value={form.ends_at} onChange={e => handleFormChange('ends_at', e.target.value)} /></div>
                </div>
                ...
                <div className="form-row-2">
                  <div className="form-row"><label>Active Time Start</label><input type="time" value={form.active_time_start} onChange={e => handleFormChange('active_time_start', e.target.value)} /></div>
                  <div className="form-row"><label>Active Time End</label><input type="time" value={form.active_time_end} onChange={e => handleFormChange('active_time_end', e.target.value)} /></div>
                </div>
              </fieldset>
