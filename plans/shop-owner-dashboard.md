...
- [x] 4.1 `productController.js` `getProducts` — the main `productQuery` (`WHERE p.available = 1 AND p.deleted = 0 AND p.is_combo = 0`). The parallel `comboQuery` is NOT filtered (combos are shop-less).
- [x] 4.2 `productController.js` `getProducts` — the offer-scoped query (`WHERE op.offer_id = ? AND op.active = 1 AND p.available = 1 AND p.deleted = 0 AND p.is_combo = 0`).
- [x] 4.3 `productController.js` `getProductById` — the single-product lookup (`FROM products p LEFT JOIN categories c ... WHERE p.id = ? AND p.deleted = 0`). Here do NOT hard-404: mirror the existing time-window pattern (`product.in_time_window = ...`) by computing the shop-open state and, when closed, responding as if `available` were 0 — set `product.available = 0` in the response object after the row is fetched, when the owning shop is closed/inactive. (Detail pages deep-linked from history should render, but show the item as unavailable.)
- [x] 4.4 `cartController.js` — the cart-preview product validation (`'SELECT id, name, price FROM products WHERE id IN (?) AND available = 1 AND deleted = 0'`). Add the fragment (alias-free: `AND (shop_id IS NULL OR EXISTS (SELECT 1 FROM shops s WHERE s.id = products.shop_id AND s.is_open = 1 AND s.active = 1))` — match the query's actual table reference). The combos SELECT next to it stays untouched.
- [x] 4.5 `orderController.js` `createOrder` — the product batch fetch modified in TASK 3.1. Same fragment. Effect: a shop closing between cart and checkout makes the product "not found," which trips the existing unavailable-product `OrderError` — exactly the desired behavior, no new error path.
- [x] 4.6 `dashboardController.js` — the two product-section queries containing `AND p.available = 1 AND p.deleted = 0 AND p.is_combo = 0` (there are two occurrences; both are customer-facing dashboard section item reads). The combo-child queries (`WHERE ci.combo_id ...`) stay untouched.
- [x] 4.7 Search the API for any OTHER customer-facing `available = 1` product read this list missed (`grep -rn "available = 1" apps/api/src`). If one exists and is customer-facing, apply the fragment and note it; if it is admin-facing, leave it and note why in the task NOTE.
  NOTE (done): Applied the shop-open filter fragment to all 6 customer-facing product reads: productController main productQuery (4.1) + offer-scoped query (4.2); getProductById uses post-fetch shop-state check setting available=0 (4.3, mirrors in_time_window pattern); cartController product validation (4.4, alias-free products.shop_id); orderController createOrder product batch (4.5, alias-free); dashboardController BOTH product_block queries at L454 & L612 (4.6, p. alias). Combos (comboQuery, combo_block, combo-child) intentionally NOT filtered — combos are shop-less house items per OUT OF SCOPE. 4.7 audit (`grep -rn 'available = 1' apps/api/src/controllers/`): all remaining `available = 1` reads are combo reads (FROM combos / combo_items) or the combo child_count subquery — all shop-less, correctly left untouched. settingsController offer-product queries are admin-facing (offer management, no available=1 filter) — left untouched. categoryController product_count subquery is customer-facing but counts by `deleted=0 AND is_combo=0` only (no `available = 1`), so not in scope per the literal "available = 1 product read" instruction. `npm test` → 44 suites, 481 passed, 1 skipped (matches baseline).
- [x] 4.8 Run `npm test` in `apps/api`.
...
**Steps:**
- [x] 6.1 API `createProduct`: `shop_id` joins the destructured `req.validatedData` fields; validate when present (must match `SELECT id FROM shops WHERE id = ?`, else 400 `VALIDATION_ERROR` `'Unknown shop_id'`); add the column + value to the INSERT (nullable). Update the product validator schema (wherever `createProduct`'s `validatedData` is defined — follow the `validate(...)` chain from `productRoutes.js`) to pass `shop_id`/`shopId` through as an optional positive integer or null.
- [x] 6.2 API `updateProduct`: same treatment — optional `shop_id` in validated data, validated against `shops`, included in the UPDATE when provided (explicit `null` clears it).
- [x] 6.3 API `getAdminProducts` and `getAdminProductById`: include `p.shop_id` and a joined `s.name AS shop_name` (`LEFT JOIN shops s ON s.id = p.shop_id`) in their SELECTs. Additive fields only.
- [x] 6.4 Admin UI: in the product create/edit form, add a "Shop" `<select>` — options from `GET /api/admin/shops` (label = shop name, value = id) plus a first option "No shop (house item)" with value empty ⇒ send `shop_id: null`. In the product list table, add a "Shop" column rendering `shop_name || '—'`.
- [x] 6.5 Run `npm test` in `apps/api` and `npm run lint` in `apps/admin`.
  NOTE (done): API — productSchema (adminRoutes.js) passes shop_id/shopId (optional positive int or null); createProduct + updateProduct destructure shop_id, validate against shops (400 'Unknown shop_id'), and include it in INSERT/UPDATE (nullable); getAdminProducts + getAdminProductById add `p.shop_id, s.name AS shop_name` via LEFT JOIN shops. Admin UI — added ShopsApi.list, fetch shops on mount, pass shops to ProductFormDrawer, Shop `<select>` after Category (empty = house item → null), Shop column in list (`shop_name || '—'`, colSpan 7→8), payload sends shop_id/shopId (|| null). Validator lives in adminRoutes.js (the validate() chain mounts there, not productRoutes.js). `npm test` (api) → 45 suites, 485 passed, 1 skipped; `npm run lint` (admin) → clean, no errors.
...
- [x] 7.3 `shopRoutes.js` — express router: `router.use(requireCustomer); router.use(requireShopOwner);` then the five routes above, each `asyncHandler`-wrapped. Mount in the app entry where the other `app.use('/api/...', ...)` lines live: `app.use('/api/shop', shopRoutes);`.
- [x] 7.4 Test file `apps/api/tests/shopOwner.test.js`: non-owner customer → 403; owner toggling another shop's product → 404; `getMyOrders` excludes `Pending` and `Delivered` orders and excludes other shops' items; `confirmMyOrder` idempotency (second call 200, timestamps unchanged). Follow existing test conventions.
- [x] 7.5 Run `npm test` in `apps/api`.
  NOTE (done): Created shopOwnerMiddleware.js (requireShopOwner, fresh DB check), shopOwnerController.js (6 handlers: getMyShop/toggleMyShop/getMyProducts/toggleMyProduct/getMyOrders/confirmMyOrder — all derive shop from req.shop, no PII/prices in orders, idempotent confirm via shop_confirmed_at IS NULL, emits admin.order.shop_confirmed), shopRoutes.js (router.use requireCustomer+requireShopOwner, 6 asyncHandler routes), mounted at /api/shop in app.js. shopOwner.test.js (5 tests: 403 non-owner, 404 wrong-shop toggle, getMyOrders status/PII filtering, idempotent confirm, 404 no-items confirm) mocks pool + realtime/socket. `npm test` → 46 suites, 490 passed, 1 skipped (was 45/485; +1 suite +5 tests, all green).
...
**Steps:**
- [x] 9.1 API: the admin order-detail items query is `SELECT * FROM order_items WHERE order_id = ?` — `shop_id` and `shop_confirmed_at` now come along for free. Extend it to also join the shop name: `SELECT oi.*, s.name AS shop_name FROM order_items oi LEFT JOIN shops s ON s.id = oi.shop_id WHERE oi.order_id = ?`. Additionally compute and add ONE new top-level field to the detail response: `shopConfirmations` — array of `{ shopId, shop_id, shopName, shop_name, confirmed (bool: every item of that shop has shop_confirmed_at), confirmedAt, confirmed_at (max timestamp or null) }`, one entry per distinct non-null `shop_id` among the items. Orders with only house/combo items get `[]`. Existing fields untouched.
- [x] 9.2 Admin UI: on the order detail view, when `shopConfirmations.length > 0`, render a badge row: per shop, name + "✓ Confirmed" (green) or "⏳ Waiting" (amber). Subscribe to the socket event `admin.order.shop_confirmed` (added in TASK 7.2) wherever the admin app already listens for `admin.order.updated`, and refetch/patch the open order's confirmations on receipt.
- [x] 9.3 Run `npm test` in `apps/api` and `npm run lint` in `apps/admin`.
  NOTE (done): API — getAdminOrderById items query now `SELECT oi.*, s.name AS shop_name ... LEFT JOIN shops`; computes `shopConfirmations` (one entry per distinct non-null shop_id, both casings, confirmed bool = all items have shop_confirmed_at, confirmedAt = max timestamp or null); orders with only house/combo items get []. Admin UI — added 'admin.order.shop_confirmed' to ADMIN_ORDER_EVENTS in realtimeClient.js (socket binds + subscribeAdminOrderEvents receives it); Orders.jsx handler refetches the selected order on receipt; badge row rendered in the Items detail section (green ✓ Confirmed / amber ⏳ Waiting). `npm test` (api) → 46 suites, 490 passed, 1 skipped; `npm run lint` (admin) → clean.
...
**Steps:**
- [x] 10.1 `utils/apiMappers.js` — `normalizeSession` must pass the new `shop` field through (both `firebaseVerify` and `getMe` responses now carry it, per TASK 2). Missing/undefined ⇒ `null`.
- [x] 10.2 `useAuthStore.js` — persist `shop` alongside `user`: set it in `setSession` (accept a third argument or read it off the normalized session — follow how `user` flows today), refresh it in the session-restore path (where `fresh.user` is applied, also apply `fresh.shop ?? null`), clear it on logout. Expose `shop` in the store state.
- [x] 10.3 `api/shopApi.js` — thin client over the TASK 7 endpoints using the existing `apiClient` with `auth: 'customer'` (same pattern as `authApi.js`): `getMyShop`, `toggleShop(isOpen)`, `getMyProducts`, `toggleProduct(id, available)`, `getMyOrders`, `confirmOrder(orderId)`.
- [x] 10.4 `navigation/ShopOwnerNavigator.js` — bottom tabs, three screens, styled with the app's existing theme components:
...
- [x] 10.5 `RootNavigator.js` — where `CustomerNavigator` is rendered, branch: `const shop = useAuthStore(s => s.shop);` → authenticated AND `shop` present ⇒ render `ShopOwnerNavigator`, else `CustomerNavigator` (unauthenticated flow unchanged — the login screens still run first). Keep the analytics `SCREEN_NAME_MAP` untouched; shop screens are not analytics-whitelisted (they'll be ignored, which is fine for v1).
- [x] 10.6 Push tap-through: `useLocalNotifications` (or wherever notification taps navigate via `navigationRef`) — when the notification payload has `data.type === 'shop_order'`, navigate to the ShopOrders tab. Follow the existing tap-navigation pattern in that hook.
- [x] 10.7 Run `npm test` in `apps/customer-app`.
  NOTE (done): 10.1 — normalizeSession passes `shop: data.shop ?? null`. 10.2 — useAuthStore adds `shop` to state, setSession (3rd arg), validateSession updates (`fresh.shop ?? null`), logout (clears), partialize (persists). AuthScreen handleSuccess + both firebaseVerify call sites pass `session.shop`. 10.3 — shopApi.js (6 methods, apiClient auth:'customer'). 10.4 — ShopOwnerNavigator (bottom tabs, theme colors) + 3 screens: ShopHomeScreen (Switch, optimistic+rollback, logout), ShopOrdersScreen (useFocusEffect poll + subscribeRealtime('shop.order.assigned') refetch + Confirm button/Confirmed state), ShopProductsScreen (availability Switch per row). 'shop.order.assigned' added to customer realtimeClient ORDER_EVENTS so the socket binds it. 10.5 — RootNavigator branches: isAuthenticated && shop → ShopOwnerNavigator, else CustomerNavigator (SCREEN_NAME_MAP untouched). 10.6 — useLocalNotifications checks data.type === 'shop_order' → navigate('ShopOrders'). No expo-haptics in deps so no sound/haptic fired (plan: "if one exists"). `npm test` (customer-app) → 11 suites, 86 passed, all green.
