...
- Commit: `feat: MAP TASK 8 — checkout interactive location picker`
  ...
  - **Superseded again 2026-07-13** (`15c04e6`, `d8483f1`): (1) pinch/pan on the inline map was being stolen by the outer page `ScrollView` — state-driven `scrollEnabled` toggling was too slow to beat the native scroll responder; switched to `setNativeProps` via a ref on touch start/end, applied synchronously. (2) Bigger change per explicit product ask: the entire Delivery Details card (header, RECOMMEND pill, option-card picker, Gorakhpur warning banner) was removed. `locationMode` now defaults to `'gps'` so the page opens directly into an edge-to-edge `LocationPicker` (`fullBleed` prop — no margin/radius, negative-margin wrapper cancels `scrollContent`'s padding). Added `onEnterManually` prop to `LocationPicker` rendering a third action in the same button row (Use my current location / Confirm location / Enter Manually). Manual mode gets a "Use map instead" link back. GPS status feedback and manual-entry validation logic untouched, just re-hung on the new layout. Verified live on device: immersive map first, 3 buttons, Enter Manually ↔ Use map instead round-trip both work.
