...
export default function () {
  group('customer browse session', () => {
    ...
    let res = http.get(`${BASE_URL}/settings`, { tags: { step: 'settings' } });
    ...
    // 2. Categories
    res = http.get(`${BASE_URL}/categories`, { tags: { step: 'categories' } });
    browseLatency.add(res.timings.duration);
    errorRate.add(res.status !== 200);
    check(res, { 'categories 200': (r) => r.status === 200 });

    // 3. Products list
    res = http.get(`${BASE_URL}/products`, { tags: { step: 'products' } });
    browseLatency.add(res.timings.duration);
    errorRate.add(res.status !== 200);
    ...
    if (productId) {
      ...
      res = http.get(`${BASE_URL}/products/${productId}`, { tags: { step: 'product_detail' } });
      browseLatency.add(res.timings.duration);
      errorRate.add(res.status !== 200);
